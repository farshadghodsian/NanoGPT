...
# See above Github issue for why we have to set this on gfx1100 GPUs before loading torch
import os
...
m = model.to(device) # move model to device to use for generate
...
context = torch.zeros((1,1), dtype=torch.long, device=device) # also create context on device
