...
class CasualSelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        # attention (materializes the large (T,T) matrix to all the queries and keys)
        att = (q @ k.transpose(-2, -1)) * (1.0 / math.sqrt(k.size(-1)))
        attn = att.masked_fill(self.bias[:,:,:T,:T] == 0, float('-inf'))
        attn = F.softmax(att, dim=-1)
        y = attn @ v # (B, nh, T, hs) -> (B, nh, T, hs)
...
class MLP(nn.Module):
    ...
    def __init__(self, config):
        ...
        slef.c_proj = nn.Linear(4 * config.n_embd, config.n_embd)
